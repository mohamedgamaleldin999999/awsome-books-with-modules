...
const withDefaultNumberingSystem = Helpers.setUnset('defaultNumberingSystem')
const withDefaultOutputCalendar = Helpers.setUnset('defaultOutputCalendar')
...
test('DateTime.fromJSDate(date) clones the date', () => {
  ...
  const oldValue = dateTime.valueOf()
...
test('DateTime.fromJSDate(date) accepts a zone option', () => {
  const date = new Date(1982, 4, 25)
  const dateTime = DateTime.fromJSDate(date, { zone: 'America/Santiago' })
...
test('DateTime.fromMillis(ms) accepts a zone option', () => {
  ...
  const dateTime = DateTime.fromMillis(value, { zone: 'America/Santiago' })
...
test('DateTime.fromSeconds(ms) accepts a zone option', () => {
  const seconds = 391147200
  const dateTime = DateTime.fromSeconds(seconds, { zone: 'America/Santiago' })
